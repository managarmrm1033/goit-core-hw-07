...
def add_birthday(args, book):
    if len(args) != 2:
        print("Invalid number of arguments for 'add-birthday' command. Usage: add-birthday [ім'я] [дата народження]")
    else:
        name, birthday = args
...
def show_birthday(args, book):
    if len(args) != 1:
        print("Invalid number of arguments for 'show-birthday' command. Usage: show-birthday [ім'я]")
    else:
        name = args[0]
        record = book.find(name)
...
def main():
    ...
    while True:
        ...
        if command in ["close", "exit"]:
            ...
            if len(args) < 2:
                print("Invalid number of arguments for 'add' command. Usage: add [ім'я] [телефон]")
            else:
                name, *phones = args
                record = book.find(name)
                if record:
            ...
            if len(args) != 2:
                print("Invalid number of arguments for 'change' command. Usage: change [ім'я] [новий телефон]")
            else:
                name, new_phone = args
                record = book.find(name)
                if record:
